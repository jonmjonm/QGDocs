...
Unlike the previous example, there is a small complication with the JSON Adjacency file [Geo/Adjacency/NC_pct21.json](NC_pct21.json). There is no field that can be used for the nodes/vertices that is a string and unique. The `id` field is a unique identifier vut it is an integer. The `prec_id` field is only unique with in each county. There are several ways to remedy this. We choose to combine the `county` with the `prec_id` to create a new field called `county_and_prec_id`.
...
Next we loop through the nodes and create the new new field called `county_and_prec_id`.
...
Lastly we load basic graph into an adjacency graph object.
```{.julia}
graph = Graph(base_graph, "county_and_prec_id");    
```

We now not place this modification in the a script that is a small modification of the script we used for Connecticut.
